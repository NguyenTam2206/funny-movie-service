...
router.post('/register', async (req, res) => {
    ...
    if (errors.length > 0) return res.json(errors);
        ...
        const hasPassword = await bcrypt.hash(password, Number(process.env.SALT_ROUNDS));
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.json({ code: 0, msg: 'Username or password is invalid.' });
        const isValidPassword = await bcrypt.compare(password, user.password);
        if (!isValidPassword) return res.json({ code: 0, msg: 'Username or password is invalid.' });
